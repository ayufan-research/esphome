...
class Display {
 public:
  ...
  virtual void draw_pixel_at(int x, int y, Color color = COLOR_ON) = 0;
...
class DisplayPage {
 public:
  ...
  DisplayPage(display_buffer_writer_t writer) : DisplayPage(
    [writer](Display& display) { return writer((display::DisplayBuffer&)display); }) {}
  void show();
  void show_next();
  void show_prev();
